use agm_core::skills::{
    SkillFile, SkillName, SkillPackage, validate_skill_relative_path,
};
...
impl<R: Read> Read for GzipReadContext<R> {
    fn read(&mut self, buffer: &mut [u8]) -> io::Result<usize> {
        self.0.read(buffer).map_err(|error| {
            io::Error::new(
                error.kind(),
                format!("Failed to read gzip stream: {error}"),
            )
        })
    }
}
...
impl ArchivePass {
    fn inspect<R: Read>(
        ...
        let counter_path = repository_path
            .as_deref()
...
fn discover_skill_root(
    ...
    for entry in entries {
        ...
        let Some(repository_path) =
            pass.inspect(&entry, limits, "discovering a skill")?
        else {
            continue;
        };

        if !entry_type.is_file()
            || repository_path.file_name() != Some(OsStr::new("SKILL.md"))
        {
            continue;
        }
    ...
    match candidates.as_slice() {
        ...
        _ => {
            let paths = candidates
                ...
                .join(", ");
            bail!(
                "Skill `{requested_name}` is ambiguous; matching SKILL.md paths: {paths}"
            )
        }
    }
}
...
fn collect_skill_files(
    ...
    for entry in entries {
        ...
        let entry_type = entry.header().entry_type();
        let Some(repository_path) =
            pass.inspect(&entry, limits, "collecting a skill package")?
        else {
            continue;
        };
...
fn read_entry_body<R: Read>(
    ...
    let observed_size = u64::try_from(bytes.len()).context("Observed archive entry size overflow")?;
...
fn normalize_repository_path(
    ...
    let first = components.next().with_context(|| {
        format!("Archive entry has an empty path: `{}`", effective_path.display())
    })?;
...
fn frontmatter_yaml<'a>(text: &'a str, repository_path: &Path) -> Result<&'a str> {
    ...
    for line in lines {
        ...
        end = end.checked_add(line.len()).context("Frontmatter offset overflow")?;
    }
...
mod tests {
    ...
    fn rejects_missing_frontmatter() {
        ...
        assert!(error.to_string().contains("must begin with YAML frontmatter"));
        assert!(
            error
                .to_string()
                .contains("skills/find-skills/SKILL.md")
        );
    }
